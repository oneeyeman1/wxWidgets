                    ...
                    else
                    {
                        if( event.GetKeyCode() == WXK_BACK )
                        {
                            ...
                            if( index != (size_t)wxNOT_FOUND
                                && GetFieldFlags( (size_t) index).GetAlignment() == wxALIGN_LEFT
                                && !CellIsEmpty( carPos ) )
                            {
                                carPos++;
                            }
...
bool wxMaskedEdit::InsertAtPos(wxChar iChar, long* pos, size_t index, wxString* inText)
{
...
void wxMaskedEdit::DoCut()
{
    ...
    wxString cutStr = ( GetTextEntry()->GetValue() ).substr( (size_t) selFrom, (size_t) ( selTo - selFrom ) );
