...
static void maxlen_handler(GtkTextBuffer*buffer, GtkTextIter *location, gchar *text, gint len, wxTextCtrl *win)
{
    ...
    if ( count > maxlen )
    {
        ...
        // Using g_sigmal_connect() doesn't work as the text is still being entered
        // probably bnecause the signal is documented as "Run Last". For exactly same
        // reason ("Run Last") it won't work in GTKOnInsertText() as its called from
...
{
    if ( GetWindowStyleFlag() & wxTE_MULTILINE )
    {
        int maxlen = GetMaxLength();
        int textlen = GetValue().length();
